...
public class SamzaPipelineTranslator {
  ...
  public static void createConfig(
    ...
    final Map<String, Map.Entry<String, String>> pTransformToInputOutputMap =
        new HashMap<>();
    ...
    try {
      ...
      objectMapper.registerModule(new SimpleModule().addSerializer(Map.Entry.class, new MapEntrySerializer()));
      configBuilder.put(
          SamzaRunner.BEAM_TRANSFORMS_WITH_IO, objectMapper.writeValueAsString(pTransformToInputOutputMap));
    } catch (IOException e) {
      LOG.error(
  ...
  public static final class MapEntryDeserializer extends JsonDeserializer<Map.Entry> {
    @Override
    public Map.Entry deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JacksonException {
